...
scatterplot(Response~logDose|Product,smooth=FALSE,reg.line=lm,data=ds)
...
d+c(-1,1)*qt(1-0.10/6,6)*sqrt(vmat[5,5]+vmat[6,6]-2*vmat[5,6])
...
# Reduced model
modelred=lm(Response~logDose+Product,data=ds)
summary(modelred)
anova(modelred)
